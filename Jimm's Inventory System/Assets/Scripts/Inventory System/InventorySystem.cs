...
public class InventorySystem : MonoBehaviour
{
    ...
    InventorySlot whereShoulditemReturn;
    [System.Serializable]
    ...
    void SetupSlots()
    {
        ...
        Invoke(nameof(AddStartingItems), .12f);
    }
    void AddStartingItems()
    {
        for (int i = 0; i < itemsToAdd.Length; i++)
        {
            var slot = slots[itemsToAdd[i].slotIndex];
            //AddItem(itemsToAdd[i].item, itemsToAdd[i].amount);
            slot.Item = new InventoryItem() { item = itemsToAdd[i].item, amount = itemsToAdd[i].amount };
            slot.UpdateSlot();
    ...
    public void SwapWithMouse(InventorySlot slot)
    {
        ...
        //AudioManager.instance.Play("SlotClick");
        ...
        if (MouseSlot.Item.item != null) whereShoulditemReturn = slot;
    }
    /// <summary>
    ...
    {
        if ((MouseSlot.Item.item == null || MouseSlot.Item.amount < 1) || (whereShoulditemReturn == null && whereShoulditemReturn != MouseSlot)) return;
        SwapWithMouse(whereShoulditemReturn);
        whereShoulditemReturn = null;
    }
