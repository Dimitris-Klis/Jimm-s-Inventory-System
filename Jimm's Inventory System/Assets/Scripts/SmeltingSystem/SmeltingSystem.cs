...
public class SmeltingSystem : MonoBehaviour
{
    ...
    public SmeltingRecipe[] GetRecipes(string workstationName)
    {
        List<SmeltingRecipe> rs = new();
        for (int i = 0; i < recipes.Length; i++)
        {
            var r = recipes[i];
            if (r.RequiredWorkstation == workstationName)
            {
                rs.Add(r);
            }
        }
    ...
    /// </summary>
    public void UpdateSlotsUI(Smelter smelter)
    {
        if (!openSmelter) return;
        InputSlot.Item = smelter.Input;
        InputSlot.UpdateSlot();
